...
from utilrnn import init_weight, get_robert_frost
...
class SimpleRNN:
    ...
    def fit(self, X, learning_rate=0.0001, mu=0.99, reg=1.0, activation=T.tanh, epochs=1500, show_fig=False):
        N = len(X)
        D = self.D
        M = self.M
        V = self.V
        ...
        self.hz= theano.shared(hz)
        self.Wo = theano.shared(Wo)
        self.bo = theano.shared(bo)
        self.params = [self.We, self.Wx, self.Wh, self.bh, self.h0,self.Wxz,self.Whz,self.hz, self.Wo, self.bo]
        ...
        def recurrence(x_t, h_t1):
            # returns h(t), y(t)
            h_hat = self.f(x_t.dot(self.Wx) + h_t1.dot(self.Wh) + self.bh)
            z= T.nnet.sigmoid(x_t.dot(self.Wxz)+h_t1.dot(self.Whz) +self.hz)
    ...
    def load(filename, activation):
        ...
        _, M = Wx.shape
        rnn = SimpleRNN(D, M, V)
        rnn.set(We, Wx, Wh, bh, h0,Wxz,Whz,hz, Wo, bo, activation)
        return rnn
    ...
    def generate(self, pi, word2idx):
        ...
        # since the strat symbol will yield the same word
...
def train_poetry():
    ...
    rnn.fit(sentences, learning_rate=1e-4, show_fig=True, activation=T.nnet.relu, epochs=1500)
